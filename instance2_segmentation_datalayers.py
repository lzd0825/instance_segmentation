...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        for label in unique_labels:
            if label == 0:
                continue
            orientation_map = np.zeros((self.im_shape[0], self.im_shape[1])).astype(np.float32)
            delta_x_map = x_coords - centroids[label, 1]
            delta_y_map = y_coords - centroids[label, 0]
