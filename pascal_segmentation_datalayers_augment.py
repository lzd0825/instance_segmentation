...
class BatchLoader(object):
    ...
    def load_next_image(self):
        ...
        # Random crop an image and resize if the original image is larger
        if self.im_shape[0] < im.shape[0]:
            height = random.randint(self.im_shape[0], im.shape[0])
            y = random.randint(0, im.shape[0] - height)
        else:
            height = im.shape[0]
            y = 0
        if self.im_shape[1] < im.shape[1]:
            width = random.randint(self.im_shape[1], im.shape[1])
